"""
Pre-compute and cache DINOv2 features for all MVTec AD categories.

Extracts features ONCE and saves as .npy files. All experiments then
load from cache instead of re-extracting, reducing total time from
~2hrs to ~30min for initial extraction + seconds for loading.

Usage:
    .venv\\Scripts\\python.exe scripts\\precompute_features.py

Output: data/features/dinov2_vitb14/<category>/train_features.npy
                                              /test_features.npy
                                              /test_labels.npy
"""
import sys, os, time
...
def extract_and_save(backbone, category, feature_dir):
    ...
    # Save as list of arrays (variable-sized per image is same here)
...
def main():
    print("="*60)
    print("Pre-computing DINOv2 features for MVTec AD")
    print("="*60)
